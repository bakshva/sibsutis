...
void calculate_forces(struct particle *p, struct particle *f, float *m, int n, omp_lock_t *locks) {
...
int main(int argc, char *argv[]) {
    ...
    omp_lock_t *locks = malloc(sizeof(omp_lock_t) * n);
    for (int i = 0; i < n; i++)
    ...
    #pragma omp parallel
    {
        for (double t = 0; t <= 1; t += dt) { 
            ...
            calculate_forces(p, f, m, n, locks); 
            #pragma omp barrier
            if (omp_get_thread_num() == 0) tforces += wtime();
    ...
    ttotal = wtime() - ttotal;
    printf("# NBody Version 3: n Locks (n=%d, threads=%d)\n", n, omp_get_max_threads());
